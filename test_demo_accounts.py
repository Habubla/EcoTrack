"""
Test script to verify all demo accounts work correctly with the seed_demo.py script.
...
def test_demo_files_exist():
    ...
    if not demo_data_dir.exists():
        print("❌ Demo data directory does not exist")
        return False
    ...
    print("✅ All demo CSV files exist")
    return True
    ...
    for filename in ["solar_profile.csv", "campus_profile.csv", "admin_profile.csv",
        ...
        try:
            ...
            if not pd.api.types.is_numeric_dtype(df["power_kw"]):
                print(f"❌ {filename} power_kw column is not numeric")
                return False

            if not pd.api.types.is_numeric_dtype(df["cumulative_kwh"]):
                print(f"❌ {filename} cumulative_kwh column is not numeric")
                return False

            # Check row count
            if len(df) != 24:
                print(f"❌ {filename} should have exactly 24 rows, got {len(df)}")
                return False

            print(f"✅ {filename} - Valid format ({len(df)} rows)")
    ...
    for filename, ranges in expected_ranges.items():
        ...
        try:
            ...
            if min_val < ranges["min"] or max_val > ranges["max"]:
                print(f"❌ {filename} data out of expected range [{ranges['min']}, {ranges['max']}]")
                print(f"   Actual range: [{min_val:.1f}, {max_val:.1f}]")
                return False

            print(f"✅ {filename} - Data within realistic range [{min_val:.1f}, {max_val:.1f}]")
...
def test_processing_function():
    ...
    for filename in ["solar_profile.csv", "campus_profile.csv", "admin_profile.csv",
        ...
        try:
            ...
            if summary["rows"] != 24:
                print(f"❌ {filename} should have 24 rows")
                return False

            if summary["mean_load"] <= 0:
                print(f"❌ {filename} mean load should be positive")
                return False

            print(f"✅ {filename} - Processing function works correctly")
...
def main():
    ...
    if all_passed:
        print("🎉 All tests passed! Demo accounts are ready to use.")
        return 0
    else:
